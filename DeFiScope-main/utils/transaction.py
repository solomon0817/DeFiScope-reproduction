...
class Transaction:
    ...
    def get_abi(self, address: str) -> Dict:
        # [Fix 1] Etherscan V2 API 统一入口
        v2_endpoint = "https://api.etherscan.io/v2/api"
        
        # [Fix 2] 增加 Headers 伪装浏览器 (防止 403 Forbidden)
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
        }

        # 1. 确定 Chain ID 和使用的 API Key
        # 注意：V2 接口统一使用 Etherscan 的 API Key
        if self.platform == "ethereum":
            chain_id = "1"
            # 确保 config.py 里 ethereum 的 api_key 是有效的 Etherscan Key
            api_key = SUPPORTED_NETWORK["ethereum"]["api_key"]
        elif self.platform == "bsc":
            chain_id = "56"
            # 强制使用 Ethereum 的 Key (因为旧的 BSC Key 不通用)
            api_key = SUPPORTED_NETWORK["ethereum"]["api_key"]
        elif self.platform == "fantom":
            chain_id = "250"
            api_key = SUPPORTED_NETWORK["ethereum"]["api_key"]
        else:
            print(f"[!]Unsupported platform for V2 API: {self.platform}")
        ...
        try:
            # [Fix 2] 传入 headers
            response = requests.get(abi_endpoint, headers=headers, timeout=10) # 加上超时时间
            
            if response.status_code != 200:
                print(f"[!] HTTP Error {response.status_code} fetching ABI for {address}")
                return {"status": "0", "result": []}
                
            abi = json.loads(response.text)
            
            # [Fix 3] 检查 API 是否返回逻辑错误 (如 Invalid API Key)
            if abi.get("status") == "0" and abi.get("message") == "NOTOK":
                print(f"[!] API Error for {address}: {abi.get('result')}")
                return {"status": "0", "result": []}
                
            return abi
        except Exception as e:
            print(f"[!]Error fetching ABI for {address}: {e}")
            return {"status": "0", "result": []}
